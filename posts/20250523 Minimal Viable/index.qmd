...
leaflet() %>%
  addTiles() %>%
  addMarkers(lng = 126.70201641088923, lat = 37.49134825570369, popup = "화랑농장")
```
...
When we moved from 부평 to 주안, we got the running water, a black rotary phone (one-digit for the station, and 4 individually assigned number)^[according to my sibling our number was3-3204], and a black and white TV that pulled in all three major TV (MBC, TBC, and KBS) stations[^3] and the forerunner to AFKN broadcasting[^4]
...
However, my parents ran into a difficult stretch financially. My mom suggested he take a break from study and volunteer for a military service.
